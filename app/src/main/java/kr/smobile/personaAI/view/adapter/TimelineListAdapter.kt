...
class TimelineListAdapter : RecyclerView.Adapter<BaseViewHolder>() {
    ...
    inner class TimelineListViewHolder(var itemListTimelineBinding: ItemListTimelineBinding) :
        ...
        override fun onBind(position: Int) {
            ...
            itemListTimelineBinding.topPanel.setOnClickListener {
                var intent = Intent(itemListTimelineBinding.root.context, UserProfileActivity::class.java)
                intent.putExtra("color", 2)
                intent.putExtra("userId", timeline.user!!.path)
                itemListTimelineBinding.root.context.startActivity(intent)
            }
